...
#  环境准备（conda、NCCL等）
# ---------------------------
if [ -f /jizhicfs/fenghengyi/miniconda3/etc/profile.d/conda.sh ]; then
  source /jizhicfs/fenghengyi/miniconda3/etc/profile.d/conda.sh
  conda activate innov || true
fi

# Transformer Engine requires CUDA include dir and correct libstdc++
export NVTE_CUDA_INCLUDE_DIR=${NVTE_CUDA_INCLUDE_DIR:-/usr/local/cuda/include}
export LD_LIBRARY_PATH=/jizhicfs/fenghengyi/miniconda3/envs/innov/lib:${LD_LIBRARY_PATH:-}
...
# 项目路径 / 参数
# ---------------------------
export AIAK_TRAINING_PATH="${AIAK_TRAINING_PATH:-/jizhicfs/fenghengyi/Innovator-VL}"
...
DATA_PATH=${DATA_PATH:-"/jizhicfs/fenghengyi/data/llava_pretrain"}  # 23988 packed samples
TOKENIZER_PATH=${TOKENIZER_PATH:-"/jizhicfs/fenghengyi/innovator_saves/step1/Innovator-VL-8B-stage0"}
CHECKPOINT_PATH=${CHECKPOINT_PATH:-"/jizhicfs/fenghengyi/innovator_saves/step1/Innovator-VL-8B-stage0_mcore_tp${TP}_pp${PP}"}

# if resume from checkpoint, set the checkpoint path
RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-"/jizhicfs/fenghengyi/innovator_saves/step1/stage_1_alignment_innovator_vl_8b"}

# set the save checkpoint path and tensorboard path
SAVE_CKPT_PATH=${SAVE_CKPT_PATH:-"/jizhicfs/fenghengyi/innovator_saves/step1/stage_1_alignment_innovator_vl_8b"}
