#!/usr/bin/env bash
# ============================================================================
# Stage 1 Alignment training for Innovator-VL 30B-A3B (Qwen3-MoE backbone)
# Adapted from train_auto_stage_1_alignment_innovator_vl_8b.sh
#
# Differences vs the 8B version:
#   - --model-name             innovator-vl-30b-a3b
#   - convert script           convert_30b_a3b_hf_to_mcore.sh
#   - default parallelism      TP=1 PP=1 EP=8       (matches the mcore checkpoint layout)
#   - extra MoE megatron flags (--num-experts, --moe-*, --expert-model-parallel-size)
#   - short-verification defaults (NSTEP=10, GBS=8, SEQ_LEN=4096, no resume)
#
# Usage (short verification)
# --------------------------
...
NSTEP="${6:-2}"          # number of steps (verification default)
SAVE_INTERVAL="${7:-50}" # save interval (>= NSTEP for verification = no save)
...
DATA_PATH=${DATA_PATH:-"/jizhicfs/fenghengyi/data/llava_pretrain_energon"}
...
# Resume disabled by default for the verification run
RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-""}

# Save target
SAVE_CKPT_PATH=${SAVE_CKPT_PATH:-"/jizhicfs/fenghengyi/innovator_saves/step1/stage_1_alignment_innovator_vl_qwen3moe"}
...
# Launch
# ---------------------------
echo "Launching Stage 1 Alignment (Qwen3-MoE) with torchrun:"
