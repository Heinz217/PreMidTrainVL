...
def maybe_save_dataloader_state(train_iterator, iteration, dataloader_save_path):
    ...
    # NOTE: Each (writing) rank's data_state_save_path resolves to its own
    # mp_rank_/ep_rank_ sub-directory via get_checkpoint_name(...), so rank-0
    # alone cannot create those directories for the other ranks. Each rank
    # must ensure its own parent directory exists. exist_ok=True is implicit
    # in ensure_directory_exists, which uses os.makedirs(..., exist_ok=True).
...
def load_checkpoint(model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    if not skip_load_to_model_and_opt:
        def _reconcile_extra_states(model_sd):
            """Fix ``_extra_state`` key mismatches and format issues.

            1. Relocate keys with DDP wrapper prefixes (``module.module.``) to
               their un-prefixed form.
            2. Convert any BytesIO-wrapped ``_extra_state`` values to empty Tensors.
               Both TE implementations (``module/base.py`` and ``ops/op.py``) treat
               an empty Tensor as a no-op: ``state.numel() == 0 → return``.
               BytesIO is only handled by ``module/base.py``, causing
               ``AttributeError: '_io.BytesIO' has no attribute 'numel'`` in
               ``ops/op.py``.
            3. Drop keys that cannot be matched (would cause "unexpected key"
               errors).
            """
            import io as _io
            _WRAPPER_PREFIXES = ('module.module.', 'module.')
            _EMPTY = torch.tensor([], dtype=torch.uint8)

            for ckpt_key in list(model_sd.keys()):
                if not (ckpt_key.endswith('._extra_state') or ckpt_key == '_extra_state'):
                    continue
                v = model_sd[ckpt_key]

                # Step A: normalise format – BytesIO → empty Tensor
                if isinstance(v, _io.BytesIO):
                    model_sd[ckpt_key] = _EMPTY
                    v = _EMPTY

                # Step B: strip DDP wrapper prefix if present
                stripped = ckpt_key
                for pfx in _WRAPPER_PREFIXES:
                    if stripped.startswith(pfx):
                        stripped = stripped[len(pfx):]
                        break
                if stripped != ckpt_key:
                    if stripped not in model_sd:
                        model_sd[stripped] = model_sd.pop(ckpt_key)
                        print_rank_0(f'  relocated TE extra_state: {ckpt_key} -> {stripped}')
                    else:
                        model_sd.pop(ckpt_key)
                        print_rank_0(f'  dropped duplicate TE extra_state: {ckpt_key}')

        def _check_weights_loaded(model_sd, ref_module):
            """Warn if any non-extra-state parameter/buffer was not loaded."""
            _WRAPPER_PREFIXES = ('module.module.', 'module.')
            missing = []
            ...
            if missing:
                print_rank_0(f'WARNING: {len(missing)} parameter(s) not found in checkpoint:')
                for k in missing[:10]:
                    print_rank_0(f'  {k}')

        if len(ddp_model) == 1:
            _reconcile_extra_states(state_dict['model'])
            _check_weights_loaded(state_dict['model'], ddp_model[0])
            ddp_model[0].load_state_dict(state_dict['model'], strict=False)
        else:
            for i in range(len(ddp_model)):
                mpu.set_virtual_pipeline_model_parallel_rank(i)
                _reconcile_extra_states(state_dict['model%d' % i])
                _check_weights_loaded(state_dict['model%d' % i], ddp_model[i])
                ddp_model[i].load_state_dict(state_dict['model%d' % i], strict=False)
